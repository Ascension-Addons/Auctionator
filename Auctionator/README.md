...
- **Buying quality-of-life:**
  - **Chain Buy** (checkbox next to the Buy button): after each purchase the buy dialog stays open and hops straight to the next cheapest auction — no closing and re-clicking rows. A running **purchase summary** on the dialog shows `Bought N (M items) for: Xg Ys Zc`; totals reset when you start a new manual buy, and a chat summary prints when the chain runs out of auctions. Cancel always stops the chain.
  - **Real quantity tracking:** the summary counts actual items bought (sum of stack sizes), not just auction rows.
  - **Rollback on failed buys:** purchases are confirmed against your actual gold movement; if the server rejects a bid ("Item not found" — someone sniped it — or insufficient funds), the spent/quantity totals and the bought counter are rolled back so the summary never claims gold you didn't spend.
